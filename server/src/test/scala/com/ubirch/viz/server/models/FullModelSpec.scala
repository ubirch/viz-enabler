...
class FullModelSpec extends FeatureSpec with LazyLogging with Matchers with BeforeAndAfterEach with ConfigBase {
  ...
  def getAllDocuments: List[String] = embeddedElastic.fetchAllDocuments().asScala.toList
